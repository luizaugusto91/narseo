            ...
            try
              ...
              if qPagamentoregistra_venda.AsBoolean = True then
                begin
                  ...
                  qSQL.Open;
                  qConsulta.Edit;
                  Cupom := qSQL.FieldByName('gera_cupom_venda').AsInteger;
                  qConsultacupom.asInteger := Cupom;
                  if qPagamentopagamento.AsBoolean = True then
                ...
                begin
                  ...
                  while cdsFaturas.Eof = false do
                    begin
                      ...
                      qContasReceber.FieldByName('data_movimento').AsDateTime:= Now;
                      ...
                      qContasReceber.FieldByName('valor').AsCurrency         := cdsFaturas.FieldByName('valor_parcela').AsCurrency;
                      qContasReceber.FieldByName('parcela').AsInteger        := cdsFaturas.FieldByName('parcela').AsInteger;
                      qContasReceber.FieldByName('valor_corrigido').AsCurrency:= cdsFaturas.FieldByName('valor_total').AsCurrency;
                      qContasReceber.FieldByName('total_parcelas').AsInteger:= cdsFaturas.FieldByName('total_parcelas').AsInteger;
                      qContasReceber.FieldByName('tipo_pagamento').AsInteger := qConsulta.FieldByName('tipo_pagamento').AsInteger;
                      qContasReceber.FieldByName('valor_original').AsInteger:= cdsFaturas.FieldByName('total_parcelas').AsInteger;
                      qContasReceber.FieldByName('centro_custo').AsString    := Parametros.cdsRegrasNegocio.FieldByName('vendas_centrocustos').AsString;
                      qContasReceber.FieldByName('vencimento').AsDateTime    := cdsFaturas.FieldByName('vencimento').AsDateTime;
                      qContasReceber.FieldByName('pessoa').AsInteger         := qConsulta.FieldByName('cliente').AsInteger;
                      qContasReceber.FieldByName('descricao').AsString       := 'VENDA N:' + qConsulta.FieldByName('codigo').AsString;
                      if qPagamentopagamento.AsBoolean = True then
                        begin
                          qContasReceber.FieldByName('status').AsString      := 'Q';
                          qContasReceber.FieldByName('quitada').AsBoolean    := True;
                        end
                      else
                        begin
                          qContasReceber.FieldByName('status').AsString      := 'A';
                          qContasReceber.FieldByName('quitada').AsBoolean    := False;
                        end;
                      qContasReceber.FieldByName('cupom').AsInteger          := Cupom;
                      qContasReceber.Post;
                      cdsFaturas.Next;
                    end;
                end;
