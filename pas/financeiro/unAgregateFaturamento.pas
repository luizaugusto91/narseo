...
begin
  ...
  if Pergunta('WHAT','Confirmação','O valor total do faturamento sera de R$ ' + FloatToStr(Total)+'.'+#13+'Confirma a geração do Faturamento?','SN') = ID_YES then
    begin
      ...
      Descricao := 'Relação de faturas agregadas no periodo de ' + DateToStr(edtdatainicio.Date) + ' a ' + DateToStr(edtdatafinal.Date) + '.';
      qContas.Close;
      qContas.SQL.Clear;
      qContas.SQL.Text := 'UPDATE contas_receber SET descricao = ' + QuotedStr(Descricao) + ' WHERE codigo = ' + IntToStr(Fatura);
      qContas.Execute;
      if Fatura <> 0 then
        begin
          try
