...
begin
  ...
  if cbbStatus.Properties.Items[cbbStatus.ItemIndex].Value = 'A' then
    qConsulta.SQL.Add(' and status in (' + QuotedStr('E') + ',' + QuotedStr('C') + ') ');
  if cbbStatus.Properties.Items[cbbStatus.ItemIndex].Value = 'E' then
    qConsulta.SQL.Add(' and status = ' + QuotedStr('E') + ' ');
  if cbbStatus.Properties.Items[cbbStatus.ItemIndex].Value = 'C' then
  ...
  qConsulta.SQL.Add(' ORDER BY conhecimento ASC');
  qConsulta.Open;
...
begin
  ...
  dtpFinal.DateTime := EndOfTheMonth(Date);
end;
