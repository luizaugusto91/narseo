...
begin
  //
  //Aviso('INFO','Não disponível.','A configuração de conexão com o servidor não esta disponivel diretamente pelo login nesta versão.');
  Connection.qEmpresas.Close;
...
begin
  ...
  Connection.qLogin.Open;
    if Connection.qLogin.RecordCount = 1 then
      ...
      begin
        Aviso('INFO','Login incorreto','Usuário ou senha incorretos. Por favor, verifique novamente.');
        edtLogin.Clear;
        edtPassword.Clear;
        edtLogin.SetFocus;
      end;
