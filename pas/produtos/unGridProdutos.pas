...
begin
  inherited;
  if Pergunta('WHAT','Atenção','Tem certeza que deseja replicar o registro selecionado?','SN') = ID_YES then
    begin
      qSQL.Close;
      ...
      try
        ...
        if qSQL.FieldByName('set_replica').AsBoolean = True then
          ...
          begin
            Aviso('WARN','Atenção','Não foi possivel replicar o registro selecionado.'+#13+'Por favor, entre em contato com o suporte.');
          end;
        qSQL.Close;
        qSQL.SQL.Clear;
        ds.DataSet.Refresh;
      except
        Aviso('ERRO','Atenção','Não foi possivel replicar o registro selecionado.'+#13+'Por favor, entre em contato com o suporte.');
      end;
    end;
end;
