...
var
  frmSystemSplash: TfrmSystemSplash;

implementation

{$R *.dfm}
uses unAplicacao, Vcl.Clipbrd, unCadConfigEmpresa, unSEFAZ;

procedure TfrmSystemSplash.FormShow(Sender: TObject);
begin
  lblPrecesso.Caption := 'Versão ' + Versao;
end;

procedure TfrmSystemSplash.PermissaoUsuarios;
var
...
begin
  ...
  try
    ...
    if Connection.qAssinatura.RecordCount = 0 then
      begin
        if Pergunta('WHAT','Configuração pendente','Não encontramos os registros da Assinatura Digital.'+#13+'Deseja realizar a configuração agora?','SN') = ID_YES then
          begin
  ...
  ProgressBar.Max := Aplicacao.ComponentCount;
  ...
  for I := 0 to Aplicacao.ComponentCount -1 do
    begin
      ProgressBar.Position := i;
      if Aplicacao.Components[i] is TAction then
        TAction(Aplicacao.Components[i]).Enabled := False;
    end;
    ...
    begin
      ...
      if Aplicacao.Components[i] is TAction then
        begin
          ...
          if TAction(Aplicacao.Components[i]).Tag = 0 then
            begin
              TAction(Aplicacao.Components[i]).Enabled := True;
              TAction(Aplicacao.Components[i]).Visible := True;
            end
          // > Caso contrario, verifica permissão.
          else
            begin
              FrmTag := TAction(Aplicacao.Components[i]).tag;
              Connection.qPermissoesUser.Close;
...
procedure TfrmSystemSplash.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmSystemSplash.Timer2Timer(Sender: TObject);
begin
