...
begin
  inherited;
  if (Pergunta('WHAT','Informação','Tem certeza que deseja fechar o caixa?','SN') = ID_YES) then
    begin
      // Fecha o caixa atual.
      qSQL.Close;
      qSQL.SQL.Clear;
      ...
      if qSQL.FieldByName('set_fechamento_caixa').AsBoolean = True then
        begin
          Aviso('INFO','Informação','Caixa finalizado com sucesso.');
          Close;
        end
      else
        begin
          Aviso('ERRO','Informação','Não foi possivel finalizar o caixa atual.'+#13+'Por favor, entre em contato com o Suporte!');
        end;
